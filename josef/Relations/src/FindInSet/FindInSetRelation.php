...
abstract class FindInSetRelation extends HasOneOrMany {
    ...
    public function addConstraints()
    {
        if (static::$constraints) {
            //whereRaw("'$farm->id' = ANY (string_to_array(user_farms,','))");
            //$this->query->whereRaw(  'FIND_IN_SET(' . $this->foreignKey . ',"' .$this->getParentKey() .'")');
          $this->query->whereRaw(  "'".$this->getParentKey()."' = ANY(string_to_array('".$this->foreignKey."',','))");
        }
    }
    ...
    public function addEagerConstraints(array $models)
    {
        ...
        //$this->query->whereRaw(  "'".implode( ',', $this->getKeys($models, $this->localKey))."' = ANY(string_to_array('".$this->foreignKey."',','))");
    }
